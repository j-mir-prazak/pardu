...
function pdl2ork() {
	var pd = spawner.spawn("DISPLAY=:1", new Array("bash", "-c", "pd-l2ork -open pd/pardu_player.pd -send \"ard "+ arduino + "\""), {detached: true})
	var decoder = new StringDecoder('utf-8')
	pids.push(pd["pid"])
...
function qlcplus() {
	var qlc = spawner.spawn("DISPLAY=:1", new Array("bash", "-c", "qlcplus -o ./scene.qxw -p"), {detached: true})
	var decoder = new StringDecoder('utf-8')
	pids.push(qlc["pid"])
